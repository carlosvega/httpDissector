...
static void NDLTclose_last(NDLTdata_t *data)
{
	if (data->interfaces != NULL) {
		for (int i = 0; i < data->numInterfaces; i++) {
			if (data->fileFormato == NDLTFORMAT_PCAP) { //si la traza es pcap
				...
				if (data->interfaces[i].trazaAbierta.ph != NULL) {
					pcap_close((data->interfaces[i].trazaAbierta.ph));
				}

			} else {
				//fclose(data->traceFile.fh);
				if (data->interfaces[i].trazaAbierta.fh) {
					fclose(data->interfaces[i].trazaAbierta.fh);
				}
			}
	...
	if (data->pcapFilterString) {
		NDLTfreecode(&data->filtroPcapCompilado);
	}
}
...
static int NDLTopen_next(NDLTdata_t *data, char *errbuf)
{
	if (data->multiFile) {
		if (!feof(data->fileOfPaths)) {
			...
			if (fgets(aux, TAM_LINE, data->fileOfPaths) == NULL) {
				return 0;
			}

			if (sscanf(aux, "%s", data->path) != 1) {
				if (errbuf) {
					sprintf(errbuf, "Error: no se pudo leer la siguiente línea del fichero");
				}
		...
		if (data->traceFile.fh || data->traceFile.ph) {
			return 0;
		}
	}
	...
	if (data->pcapFilterString) { //si hay un filtro
		if (NDLTcompile(CAPLEN, DLT_EN10MB, &data->filtroPcapCompilado, data->pcapFilterString, 1, 0)) {
			if (errbuf) {
				sprintf(errbuf, "Error: no se pudo compilar el filtro %s", data->pcapFilterString);
			}
...
static int checkPacketInFile(NDLTdata_t *trazas)
{
	...
	if (!trazas->filePacketsToDiscard) {
		return 0;    // Si no hay fichero de descartes devolvemos siempre como si el paquete no hubiera que descartarlo
	}
...
static int leerPaqueteFile(NDLTdata_t *trazas, int indice)
{
	...
	if (fread(&headRaw, 1, sizeof(header_RAW_t), trazas->interfaces[indice].trazaAbierta.fh) != sizeof(header_RAW_t)) {
		if (feof(trazas->interfaces[indice].trazaAbierta.fh)) {
			return 2;
		}
	...
	if (headRaw.caplen > 0) {
		...
		if (trazas->filePacketsToDiscard != NULL && checkPacketInFile(trazas) == -1) {
			return(-1);
		}
	...
	if (data->multiFile) {
		...
		for (int i = 0; i < data->numInterfaces; i++) {
			if (data->fileFormato == NDLTFORMAT_PCAP) {
				...
				if (retLeerPaquete == 0 || retLeerPaquete == -1) {
					return 0;
				}
			}
...
NDLTdata_t *NDLTabrirTraza(char *path, char *format, char *filter, int multi, char *errbuf)
{
	if (!path) {
		if (errbuf) {
			sprintf(errbuf, "Error: especifique una ruta a un fichero");
		}
	...
	if (multi) {
		...
		if (!fileOfPaths) {
			if (errbuf) {
				sprintf(errbuf, "Error: no se pudo abrir el fichero");
			}
	...
	if (!format || strcmp(format, NDLTFORMAT_PCAP_STR) == 0) {
		formato = NDLTFORMAT_PCAP;

	} else if (strcmp(format, NDLTFORMAT_DRIV_STR) == 0) {
		formato = NDLTFORMAT_DRIV;
	...
	if (!nuevo) {
		if (errbuf) {
			sprintf(errbuf, "Error creando objeto de tipo NDLTdata_t");
		}
		...
		while (!feof(fileOfPaths)) {
			if (fgets(aux, TAM_LINE, fileOfPaths) != NULL) {
				if (lineaVacia && strlen(aux) > 1) {
					...
					if (strlen(aux) == 1) {
						lineaVacia = 1;    //si es una línea en blanco activo el flag lineaVacia
...
NDLTdataEscritura_t *NDLTabrirTrazaEscritura(char *pathOutput, char *formatOutput, int displayOutput, unsigned int snaplen, char *errbuf)
{
	...
	if (!formatOutput || strcmp(formatOutput, NDLTFORMAT_PCAP_STR) == 0) {
		formatoSalida = NDLTFORMAT_PCAP;

	} else if (strcmp(formatOutput, NDLTFORMAT_DRIV_STR) == 0) {
		formatoSalida = NDLTFORMAT_DRIV;
	...
	if (!displayOutput) {
		strncpy(nuevo->path, pathOutput, TAM_LINE);
	}
	...
	if (nuevo->fileFormato == NDLTFORMAT_PCAP) {
		...
		if (nuevo->displayOutput) {
			nuevo->traceFile.pdh = pcap_dump_open(pcap_open, "-");

		} else {
			nuevo->traceFile.pdh = pcap_dump_open(pcap_open, nuevo->path);
		}
		...
		if (codigoResult == -1) {
			...
			if (trazas->pcapFilterString != NULL) {
				if (NDLTfilter(&trazas->filtroPcapCompilado, &pkthdr, packet)) {
					if (trazas->nextPacketActive == 1) {
						return 2;    //para salir del bucle si se ha llamado a NDLTnext()

					} else {
						callback(user, &pkthdr, packet);    //No se descarta tampoco por filtro asi que se hace el callback
					}
			...
			trazas->numPacketsDiscarded++;    //se aumenta el contador de descartados
		}
	}
...
static int checkFin(NDLTdata_t *trazas)
{
	for (int i = 0; i < trazas->numInterfaces; i++) {
		if (trazas->interfaces[i].leidosTodos != 1) {
			return 0;
		}
	}
	...
	while (1) {
		...
		if (trazas->fileFormato == NDLTFORMAT_PCAP) { //si la traza es pcap
			if (trazas->multiFile) {
				...
				if (nuevaPosALeerDeFichero > trazas->interfaces[numInterfazElegido].posALeerDeFichero) {
					trazas->bytesTotalesLeidos += nuevaPosALeerDeFichero - trazas->interfaces[numInterfazElegido].posALeerDeFichero;

				} else {
					trazas->bytesTotalesLeidos += nuevaPosALeerDeFichero;
				}
				...
				while (pcap_next_ex(trazas->traceFile.ph, &hdr, &packet) != -2) { //leer paquete
					...
					if (nuevaPosALeerDeFichero > posALeerDeFichero) {
						trazas->bytesTotalesLeidos += nuevaPosALeerDeFichero - posALeerDeFichero;
			...
			if (trazas->multiFile) {
				...
				if (resLeerPaquete == 0 || resLeerPaquete == -1) {
					return -1;
				}
				...
				while (!feof(trazas->traceFile.fh)) {
					...
					if (fread(&headRaw, 1, sizeof(header_RAW_t), trazas->traceFile.fh) != sizeof(header_RAW_t)) {
						break;
					}
					...
					if (headRaw.caplen > MAX_PACKET_LEN) {
						...
						if (trazas->numPktsLeidos == 0) {
							fprintf(trazas->fileForError, "Compruebe que el fichero no es un fichero de ficheros.\n");
						}
...
void NDLTclose(NDLTdata_t *trazas)
{
	...
	if (trazas->filePacketsToDiscard != NULL) {
		fclose(trazas->filePacketsToDiscard);
	}
...
void NDLTcloseEscritura(NDLTdataEscritura_t *trazas)
{
	...
	if (trazas->fileFormato == NDLTFORMAT_PCAP) { //si la traza es pcap
		if (!trazas->displayOutput) {
			pcap_dump_close(trazas->traceFile.pdh);
		}

	} else {
		fclose(trazas->traceFile.fh);
	}
...
FILE *NDLTfile(NDLTdata_t *trazas)
{
	...
	if (trazas->multiFile) {
		if (trazas->fileFormato == NDLTFORMAT_PCAP) {
			return pcap_file(trazas->interfaces[0].trazaAbierta.ph);

		} else {
			return trazas->interfaces[0].trazaAbierta.fh;
		}

	} else {
		if (trazas->fileFormato == NDLTFORMAT_PCAP) {
			return pcap_file(trazas->traceFile.ph);

		} else {
			return trazas->traceFile.fh;
		}
	}
}
...
unsigned long long NDLTfileSize(NDLTdata_t *trazas)
{
	...
	if (trazas->multiFile) {
		stat(trazas->interfaces[0].ficheros[trazas->interfaces[0].numficheroParaAbrir - 1], &buf);

	} else {
		stat(trazas->path, &buf);
	}
...
{
	if ((NULL == trazas) || (NULL == indexFilePath)) {
		return 0;
	}
...
static struct NDLTindexElement *NDLTgetIndexBeforePacket(NDLTdata_t *trazas, unsigned long long pktNumber) {
	...
	while (posIndice < trazas->numIndicesLeidos) {
		if (trazas->indices[posIndice].numPacket > pktNumber) {
			break;
		}
...
static void internalCallbackAvance(u_char *user, const struct NDLTpkthdr *h, const u_char *bytes)
{
	...
	if (pktActual == 1) {
		pktActual = params->pktInicial;
	}

	if (pktActual == params->pktObjetivo - 1) {
		NDLTbreakloop(params->trazas);
...
int NDLTjumpToPacket(NDLTdata_t *trazas, unsigned long long pktNumber)
{
	...
	if (eltoIndice) {
		...
		if (eltoIndice->bytesPosition != 0 && fseeko(NDLTfile(trazas), (off_t)eltoIndice->bytesPosition, SEEK_SET) == -1) {
			return 1;
		}
	}
...
void NDLTdump(NDLTdataEscritura_t *trazas, const struct NDLTpkthdr *h, const u_char *sp)
{
	if (trazas->fileFormato == NDLTFORMAT_PCAP) {
		...
		if (h->caplen < trazas->snaplen) {
			header.caplen = (u_int32_t)h->caplen;

		} else {
			header.caplen = trazas->snaplen;
		}
		...
		if (h->caplen < trazas->snaplen) {
			fwrite(&h->caplen, sizeof(u_int16_t), 1, stream);

		} else {
			fwrite(&trazas->snaplen, sizeof(u_int16_t), 1, stream);
		}

		fwrite(&h->len, sizeof(u_int16_t), 1, stream);

		if (h->caplen < trazas->snaplen) {
			fwrite(sp, sizeof(u_char), h->caplen, stream);

		} else {
			fwrite(sp, sizeof(u_char), trazas->snaplen, stream);
		}
	}
}
